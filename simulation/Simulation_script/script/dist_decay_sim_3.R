...
library(gplots)
if(file.exists("/home/wdenault/fsusi_simu/sim3/dist_decay_L_accuracy_sd3.RData")){
  load("/home/wdenault/fsusi_simu/sim3/dist_decay_L_accuracy_sd3.RData")
...
for (o  in (length(res)+1):10000) {
  ...
  for(l in 1:L){
    ...
    decay_corr_generate = function(R =2^5,# # sites
  ...
  Y <-Y+matrix(rnorm((2^5)*100 ,sd=sd(c(Y))/sqrt(3)), nrow = 100)
  ...
  save(res, file="/home/wdenault/fsusi_simu/sim3/dist_decay_L_accuracy_sd3.RData")
}
