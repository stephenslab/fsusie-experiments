...
```{r dist-to-tss, fig.height=2.5, fig.width=3}
...
The HA SNPs are strongly concentrated near gene TSSs. I couldn't find
any hard data showing a similar result, although this seems to be
suggested by the [McVicker et al (2013) paper][mcvicker-science].

```{r dist-to-tss-haqtl, fig.height=2.5, fig.width=3}
bin_size <- 25000
...
regions are similar (but not exactly the same) as the fSuSiE analyses
of HA:
...
The methylation SNPs are also strongly concentrated near the gene
TSSs, in agreement with Fig. 3 of [Ng et al 2017][xqtl-nat-neurosci]
(noting that our result is more convincing because we actually
fine-mapped the SNPs, whereas Ng et al simply chose the top SNP in
each region).

```{r dist-to-tss-mqtl, fig.height=2.5, fig.width=3}
bin_size <- 25000
bins   <- c(-Inf,seq(-2e6,2e6,bin_size),Inf)
...
Let's now examine overlap between expression, methylation and histone
acetylation SNPs simply by extracting the SNPs with PIP > 0.5 that
are also in a CS.
...
Put this information together into a table, and save this as a CSV
file for later use.

```{r create-table-of-overlapping-snps}
...
[xqtl-nat-neurosci]: https://doi.org/10.1038/nn.4632
[mcvicker-science]: https://doi.org/10.1126/science.1242429
[encode-schema-ucsc-browser]: https://genome.ucsc.edu/cgi-bin/hgTables?db=mm10&hgta_group=regulation&hgta_track=encodeCcreCombined&hgta_table=encodeCcreCombined&hgta_doSchema=describe+table+schema
