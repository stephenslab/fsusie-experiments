...
fSuSiE fine-mapping of methylation and overlap with SNPs affecting expression
-----------------------------------------------------------------------------
...
```{r fsusie-mqtl-cs-sizes, fig.height=3, fig.width=3}
...
for (i in region_names)
  cs_sizes[[i]] <-
    as.vector(table(factor(subset(fsusie_mqtl$cs,region == i)$cs)))
cs_sizes <- unlist(cs_sizes)
pdat <- data.frame(cs_size = cs_sizes)
ggplot(pdat,aes(cs_size)) +
  geom_histogram(color = "white",fill = "darkblue",bins = 64) +
