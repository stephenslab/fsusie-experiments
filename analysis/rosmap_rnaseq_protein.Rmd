...
write.table(transform(rnaseq,
                      maf = round(maf,digits = 6),
					  pip = round(pip,digits = 6)),
            "ROSMAP_DLPFC_DeJager_eQTL.cs_only.tsv",sep = "\t",
            row.names = FALSE,col.names = TRUE,quote = FALSE)
```

Note that, as discussed, we are using the "cs_coverage_0.95_min_corr"
results, which applies a purity filter of 0.8 to the 95% CSs (the
naming of this column does not make that clear).
...
write.table(transform(rnaseq,
                      maf = round(maf,digits = 6),
					  pip = round(pip,digits = 6)),
            "ROSMAP_DLPFC_Bennett_pQTL.cs_only.tsv",sep = "\t",
            row.names = FALSE,col.names = TRUE,quote = FALSE)
```
...
Save the SuSiE CSs to .rds files (these will be used for another
analysis):

```{r save-cs}
saveRDS(rnaseq,"rnaseq_susie.rds")
saveRDS(protein,"protein_susie.rds")
```
