...
sim_function= function (lev_res= 7,
  ...
  tpv2= c()
  for ( j in 20:30){
    tt= summary(lm(Y[,j]~X[,2]))$coefficients[2,4]
    tpv2= c(tpv1,tt)
...
save(res, file="/project2/mstephens/fungen_xqtl/case_study/results/test_one_SNP_difference_10cpgmvsusie.RData")


load("/project2/mstephens/fungen_xqtl/case_study/results/test_one_SNP_difference_10cpgmvsusie.RData")
tt= list()
