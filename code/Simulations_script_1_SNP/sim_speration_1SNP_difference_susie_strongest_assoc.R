...
sim_function= function (lev_res= 7,
  ...
  res= susie(y=Y[,idx_susie],X=X)

  tpv1= c()

  tt= summary(lm(Y[,idx_susie]~X[,1]))$coefficients[2,4]
  tpv1= c(tpv1,tt)



  tpv2= c()
  tt= summary(lm(Y[,idx_susie]~X[,2]))$coefficients[2,4]
  tpv2= c(tpv1,tt)
...
save(res, file="/project2/mstephens/fungen_xqtl/case_study/results/test_one_SNP_difference_susie_strongest_assoc.RData")


load("/project2/mstephens/fungen_xqtl/case_study/results/test_one_SNP_difference_susie_strongest_assoc.RData")
tt= list()
