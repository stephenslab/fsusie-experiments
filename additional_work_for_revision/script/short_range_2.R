...
if(file.exists("/home/wdenault/fsusi_simu/correlated_sim/short_range_128_sd2.RData")){
  load("/home/wdenault/fsusi_simu/correlated_sim/short_range_128_sd2.RData")
...
ell <- 20       # correlation length

d <- as.matrix(dist(1:2^S))
...
for (o  in (length(res)+1):10000) {
  ...
  for (i in 1:100) {
    eps <- drop(t(chol(Sigma)) %*% rnorm(2^S))
  ...
  m1 <- susiF(Y=Y, X=G,L=20 ,L_start=11 ,nullweight= 1,  prior="mixture_normal", cal_obj =FALSE,  maxit=10)
  m2 <- susiF(Y=Y, X=G,L=20,L_start=11 ,nullweight= 1,post_processing = "none" ,
  ...
  save(res, file="/home/wdenault/fsusi_simu/correlated_sim/short_range_128_sd2.RData")
}
