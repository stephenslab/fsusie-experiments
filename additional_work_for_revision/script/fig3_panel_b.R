...
#### Gaussian Fractionnal ---- 
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/fsusie-experiments/additional_work_for_revision/simulations/long_range_512_sd1.RData")
df_simu <- do.call(rbind, res)
...
### (The rest of your code for plotting remains the same.)
path <- getwd()
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/fsusie-experiments/additional_work_for_revision/simulations/pip_res/long_range_512_sd1_pip.RData")

 

df_susie <- data.frame(do.call(rbind, lapply( 1:length( res), function(i) res [[i]]$susie_res)))
colnames(df_susie) <- c("Number_effect", "n_cs", "n_effect", "n_false_effect",
                        "purity",
                        "cs_size")
df_simu <- df_susie
...
dfp_frac <- rbind(df_plot[, c("power", "T1_error", "prior", "L",
                         "pw_er_up", "pw_er_low", "t1_er_up", "t1_er_low",
                         "cs_size","mean_purity")],
             df[, c("power", "T1_error", "prior", "L",
                    "pw_er_up", "pw_er_low", "t1_er_up", "t1_er_low",
...
path <- getwd()
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/fsusie-experiments/additional_work_for_revision/simulations/short_range_512_sd1.RData")
df_simu <- do.call(rbind, res)
colnames(df_simu) <- c("n_cs_nps",
                       "n_effect_nps",
                       "n_false_effect_nps",
...
path <- getwd()
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/fsusie-experiments/additional_work_for_revision/simulations/pip_res/short_range_512_sd1_pip.RData")
...
df$L <- as.factor(df$L)
df$prior <- rep("SuSiE", nrow(df))
dfp_gp <-  rbind(df_plot[, c("power", "T1_error", "prior", "L",
                                "pw_er_up", "pw_er_low", "t1_er_up", "t1_er_low",
                                "cs_size","mean_purity")],
                    df[, c("power", "T1_error", "prior", "L",
                           "pw_er_up", "pw_er_low", "t1_er_up", "t1_er_low",
...
library(ggplot2)
P31 <- ggplot(dfp_gp, aes(x = L, y = power, col = prior)) +
  geom_point(position = position_dodge(.9), size = 2) +
  geom_errorbar(aes(ymin = pw_er_low, ymax = pw_er_up), width = .2,
                position = position_dodge(.9)) +
  ylim(c(0, 1)) +
...
P31_t1 <- ggplot(dfp_gp, aes(x = L, y = T1_error, col = prior)) +
  geom_hline(yintercept = 0.95, color = "black", linewidth = 2) +
  geom_point(position = position_dodge(.9), size = 2) +
  geom_errorbar(aes(ymin = t1_er_low, ymax = t1_er_up), width = .2,
                position = position_dodge(.9)) +
...
library(gridExtra)
library(grid)
titles <- list(textGrob(label = "Wavelet", gp = gpar(fontsize = 16, fontface = "bold")),
...
gaussian_frac= dfp_frac[which(as.numeric(dfp_frac$L)<15 ),] %>%
  group_by_("prior")%>% 
  summarise(power       = mean(power,na.rm=TRUE),
            coverage    = mean(T1_error, na.rm=TRUE),
            purity      = mean(mean_purity, na.rm=TRUE),
...
gaussian_gp= dfp_gp[which(as.numeric(dfp_gp$L)<15 ),] %>%
  group_by_("prior")%>% 
  summarise(power       = mean(power,na.rm=TRUE),
            coverage    = mean(T1_error, na.rm=TRUE),
            purity      = mean(mean_purity, na.rm=TRUE),
...
gaussian$sim=  "Wavelet"

block$sim = " WGBS block"
decay $sim = " WGBS decay"
gaussian_frac$sim ="Wavelet frac noise"
gaussian_gp$sim ="Wavelet GP noise"
...
#### Neww  ----
df_panel_C$sim <- factor(df_panel_C$sim, levels = c("Wavelet",
                                                    "Wavelet frac noise" ,
                                                    "Wavelet GP noise",
                                                    " WGBS block", " WGBS decay"))
...
P_power = ggplot( df_panel_C,
  ...
  scale_color_manual(values=c(  "#D41159","gold","#1A85FF"  ))

P_power
ggsave(P_power, file = paste0(save_path, "power_1_15.pdf"),
       width =20.5, height = 10.5, units = "cm")
