...
library(mvsusieR)
library(susieR)
if(file.exists("/home/wdenault/fsusi_simu/correlated_sim/spike_run_time.RData")){
  load("/home/wdenault/fsusi_simu/correlated_sim/spike_run_time.RData")
...
for (s  in 5:10) {
  ...
  for ( k in 1:10){
    ...
    lf <-  list()
    for(l in 1:L){
      lf[[l]] <-  rep(0, 2^s) #functional effect for effect l
      lf[[l]][1:5]=1
    }
    ...
    Y <- Y   +matrix(rnorm((2^s)*100 ,sd=sd(c(Y))/4), nrow = 100)
    ...
    save(res, file="/home/wdenault/fsusi_simu/correlated_sim/spike_run_time.RData")
    o=o+1
  }
}
