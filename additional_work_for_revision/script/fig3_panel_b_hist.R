...
df_1= data.frame(
  res = res_vec,
  sim_type= rep( "Wavelet",length(res_vec) ),
...
size_fsusie <-  do.call( c, lapply( 1: length(res),
                                    function( i) lengths(res[[i]]$fsusie_nps_cs)))
size_sp_fsusie <-  do.call( c, lapply( 1: length(res),
                                       function( i) lengths(res[[i]]$fsusie_ps_cs)))
...
df_2= data.frame(
  res = res_vec,
  sim_type= rep( "Wavelet frac noise",length(res_vec) ),
  Method=  rep(c("fSuSiE SPS", "fSuSiE IS","SuSiE topPC"),each=5 ),
  cs_nb=   c(rep(c(">10","1","2","3-5", "6-10"),3)))
df_2$res=2.80*df_2$res
library(ggplot2)
library(dplyr)

# Make cs_nb a factor with correct order
df_2$cs_nb <- factor(df_1$cs_nb, levels = c("1", "2", "3-5", "6-10", ">10"))

# Plot
ggplot(df_2, aes(x = cs_nb, y = res, fill = Method)) +
  geom_bar(stat = "identity", position = position_dodge( )) +
  scale_fill_brewer(palette = "Set2", name = "Method") +
  xlab("Credible Set Size") +
  ylab("Number of Credible Sets") +
...
load("C:/Document/Serieux/Travail/Data_analysis_and_papers/fsusie-experiments/additional_work_for_revision/simulations/pip_res/short_range_512_sd4_pip.RData")
size_fsusie <-  do.call( c, lapply( 1: length(res),
                                    function( i) lengths(res[[i]]$fsusie_nps_cs)))
size_sp_fsusie <-  do.call( c, lapply( 1: length(res),
                                       function( i) lengths(res[[i]]$fsusie_ps_cs)))
...
df_3= data.frame(
  res = res_vec,
  sim_type= rep( "Wavelet GP noise",length(res_vec) ),
  Method=  rep(c("fSuSiE SPS", "fSuSiE IS","SuSiE topPC"),each=5 ),
  cs_nb=   c(rep(c(">10","1","2","3-5", "6-10"),3)))
df_3$res =4.16*df_3$res
library(ggplot2)
library(dplyr)

# Make cs_nb a factor with correct order
df_3$cs_nb <- factor(df_1$cs_nb, levels = c("1", "2", "3-5", "6-10", ">10"))

# Plot
ggplot(df_3, aes(x = cs_nb, y = res, fill = Method)) +
  geom_bar(stat = "identity", position = position_dodge( )) +
  scale_fill_brewer(palette = "Set2", name = "Method") +
  xlab("Credible Set Size") +
  ylab("Number of Credible Sets") +
...
df$ sim_type <- factor(df$ sim_type , levels = c("Gaussian","Gaussian Frac noise","Gaussian GP noise", "Block", "Decay"))
...
P_out=ggplot(df , aes(x = cs_nb, y = res, fill = Method)) +
  ...
  scale_fill_manual(values=c(  "#D41159","gold","#1A85FF" ))+
  xlab("Credible Set Size") +
  ylab("Number of Credible Sets") +
  ggtitle("Comparison of Credible Set Sizes across Methods") +
  theme_minimal(base_size = 14) +
...
ggsave(P_out, 
       file = paste0(save_path, "hist_cs_size.pdf"),
