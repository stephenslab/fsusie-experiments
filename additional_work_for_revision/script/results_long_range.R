...
colors= c(  "#D41159","#FFC20A") #c(  "#FFC20A","#D41159" )


### For PVE=40% ------ 
path <- getwd()
load(paste( path,"/additional_work_for_revision/simulations/long_range_128_sd4.RData", 
            sep=""))


df_simu <- data.frame(do.call(rbind, res))
colnames(df_simu) <- c("n_cs_nps",
                       ...
                       "Number_effect",
                       "reg_sim")
#df_simu$n_effect_nps [which(df_simu$cs_size_nps>100)] <- 1
#df_simu$n_effect_ps [which(df_simu$cs_size_ps>100)] <- 1
df_simu <- df_simu[-which(df_simu$cs_size_ps>10),]
df_simu <- df_simu[-which(df_simu$cs_size_nps>10),]
library(dplyr)
...
lb_pur =min(df_plot$purity_er_low)
...
path <- getwd()
load(paste( path,"/additional_work_for_revision/simulations/long_range_128_sd1.RData", 
            sep=""))
...
### For PVE=20% ------
path <- getwd()
load(paste( path,"/additional_work_for_revision/simulations/long_range_128_sd2.RData", 
            sep=""))
...
### For PVE=30% ----- 
path <- getwd()
load(paste( path,"/additional_work_for_revision/simulations/long_range_128_sd3.RData", 
            sep=""))
...
pdf(paste0 (path, "/plot/simu_128_long_range.pdf") , width = 11.69, height = 8.27)  
...
ggarrange(P1_p,P2_p,P3_p ,
          P1_t1,P2_t1,P3_t1 ,
          P1_cs,P2_cs,P3_cs ,
          P1_pur,P2_pur,P3_pur  ,
          nrow=4,
...
ggarrange(P4_p+
            ...
            ggtitle(" PVE=30%"),P3_p+
            ggtitle(" PVE=40%"),
          P4_t1,P1_t1,P2_t1,P3_t1,
          P4_cs, P1_cs,P2_cs,P3_cs,
          P4_pur , P1_pur,P2_pur,P3_pur,
          nrow=4,
          ncol=4,common.legend = TRUE, legend="bottom")
x = c(0.01  , 0.5, 0.99,       0.5 , 0.5 , 0.5,0.25 , 0.25 ,0.25,0.75 , 0.75 ,0.75,    0.01  , 0.5, 0.99,    0.01  , 0.5, 0.99  )
y = c(0.525, 0.525, 0.525, 0.05   , 0.5 , 0.99 , 0.05   , 0.5 , 0.99, 0.05   , 0.5 , 0.99, 0.76, 0.76, 0.76, 0.285, 0.285, 0.285)
