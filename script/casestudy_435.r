...
library("cowplot")
path <- getwd()
data_list = readRDS(paste(path, "/data/Case435.rds",sep=""))
...
cowplot::plot_grid(plotlist = list(refine_effect_plot_plot,#microglia_enhancer_activity,#+facet_grid(TargetGene_name~., scales = "free"),
                                    gene_plot_plot   ,
                                   refine_plot_plot 
...
result_plot%>%ggsave(filename = paste(path, "/plot/casestudy_435_highlight.pdf", sep=""), device = "pdf",
       width = 29.7,
       height = 21,
       units = "cm"
) 