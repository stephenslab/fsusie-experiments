...
p_effect <- ggplot(effect_df, aes(x = pos)) +
  ...
  geom_line(aes(y=read_counts [1,]-read_counts [2,] ), color = "black", alpha=0.3)+
...
final_plot <- plot_grid(
  ...
  p_effect_clean,
  p_data_clean,
  p_gene,  # x-axis only shown here
  ncol = 1,
  align = "v"#,
  # axis = "lr"#,
  #rel_heights = c(1.5,1.5, 1.5, 1.5)  # Adjust heights to taste
)
...
file_path <- file.path(folder_path, "SCD_cs1.pdf")
...
# Convert x values from Mb to relative (0–1) scale for ggdraw
x_start <- (zoomin_region[1] - zoomout_region[1]) / diff(zoomout_region)
x_end   <- (zoomin_region[2] - zoomout_region[1]) / diff(zoomout_region)
...
# Add dashed vertical connectors
final_with_lines <- ggdraw(final_core) +
  geom_segment(x = 0.5, xend = 0.1,
               y = 0.755, yend = 0.7,linewidth = 2,
               linetype = "dashed", color = "black") +
  geom_segment(x = 0.5, xend = 0.91,
               y = 0.755, yend = 0.7,
