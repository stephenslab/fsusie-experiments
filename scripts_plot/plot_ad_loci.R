...
ad_loci <- data.frame(trait = c(rep("haQTL",4),rep("mQTL",11)),
                      region = c("chr1_205117782_208795513",  #  1
                                 "chr5_82637805_88412930",    #  2 (x)
                                 "chr5_85967320_89904257",    #  3 
                                 "chr8_98960936_104073051",   #  4
                                 "chr10_54746550_59267894",   #  5 (x)
                                 "chr12_109831778_113509918", #  6 (x)
                                 "chr12_111405189_114438276", #  7 (x)
                                 "chr16_21437007_24596316",   #  8 (x)
                                 "chr18_59611772_63187118",   #  9
                                 "chr18_62476957_68137751",   # 10 (x)
                                 "chr20_53859688_57519449",   # 11
                                 "chr21_24561908_27573286",   # 12 (x)
                                 "chr21_25835755_30175889",   # 13 (x)
                                 "chr3_154762617_158677749",  # 14 (x)
                                 "chr6_44880827_48309905"),   # 15
                      pos0 = c(207,86,86,100,57,
                               113,113,23,62,-Inf,
                               56,-Inf,-Inf,-Inf,47),
                      pos1 = c(208.1,89,88,101,59,
                               113.5,113.5,24,63,Inf,
                               57,Inf,Inf,Inf,47.8))
loci_to_keep <- c(1,3,4,9,11,15)
...
for (i in 1:n) {
  ...
  pos0 <- max(min(dat1$pos),min(dat2$pos),ad_loci[i,"pos0"])
  pos1 <- min(max(dat1$pos),max(dat2$pos),ad_loci[i,"pos1"])
  dat1 <- subset(dat1,pos >= pos0 & pos <= pos1)
