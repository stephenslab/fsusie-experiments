...
# Initialize
mean_func_by_snp <- function(snp_val) {
  idx <- unique(h3k9$IDsample[h3k9$SNP == snp_val])
  mean_func <- numeric(length(obs_pos))
  
  for (i in seq_along(idx)) {
    sub <- h3k9[h3k9$IDsample == idx[i], ]
    
    for (k in seq_len(nrow(sub))) {
      sub_pos <- which(obs_pos >= sub$start[k] & obs_pos <= sub$end[k])
      if (length(sub_pos) > 0) {
        mean_func[sub_pos] <- mean_func[sub_pos] + sub$normalized_reading[k]
      }
    }
    
    if (i %% 10 == 0) cat("Processed", i, "samples for SNP =", snp_val, "\n")
  }
  
  mean_func / length(idx)
}

# Compute mean functions
mean_func0 <- mean_func_by_snp(0)
mean_func1 <- mean_func_by_snp(1)

mean_func1 <- mean_func_by_snp(2)
# Save result
obs_curve <- list(
  mean_func0 = mean_func0,
  mean_func1 = mean_func1,
  mean_func2 = mean_func2,  # Assuming you define mean_func2 elsewhere
  obs_pos = obs_pos
)
